...
{
    if (x==NULL || !strcmp((char *)x->key, (char *)key))
        return x;

    if (strcmp((char *)key, (char *)x->key) < 0)
...
{
    while ((x!=NULL) && strcmp((char *)x->key, (char *)key)!=0)
    {
        if (strcmp((char *)key, (char *)x->key) < 0)
            x = x->left;
        else
 ...
 *     right 是右孩子。
 */
static AVLNode* avltree_create_node(KType key, NodeInfo *pvalue, AVLNode *left, AVLNode* right)
{
    AVLNode* p;

    if ((p = (AVLNode *)malloc(sizeof(AVLNode))) == NULL)
        return NULL;
	strncpy((char *)p->key, (char *)key, KEY_LEN);
	p->value = *pvalue;
    p->height = 0;
    p->left = left;
    p->right = right;
...
AVLNode* avltree_insert(AVLTree tree, KType key, NodeInfo *pvalue)
{
    if (tree == NULL) 
    {
        tree = avltree_create_node(key, pvalue, NULL, NULL);
    }
    else if (strcmp((char *)key, (char *)tree->key) < 0) // 应该将key插入到"tree的左子树"的情况
    {
        tree->left = avltree_insert(tree->left, key, pvalue);
        // 插入节点后，若AVL树失去平衡，则进行相应的调节。
        if (HEIGHT(tree->left) - HEIGHT(tree->right) == 2)
        {
            if (strcmp((char *)key, (char *)tree->left->key) < 0)
                tree = left_left_rotation(tree);
            else
                tree = left_right_rotation(tree);
        }
    }
    else if (strcmp((char *)key, (char *)tree->key) > 0) // 应该将key插入到"tree的右子树"的情况
    {
        tree->right = avltree_insert(tree->right, key, pvalue);
        // 插入节点后，若AVL树失去平衡，则进行相应的调节。
        if (HEIGHT(tree->right) - HEIGHT(tree->left) == 2)
        {
            if (strcmp((char *)key, (char *)tree->right->key) > 0)
...
static AVLNode* delete_node(AVLTree tree, AVLNode *z)
{
    ...
    if (strcmp((char *)z->key, (char *)tree->key) < 0)        // 待删除的节点在"tree的左子树"中
    {
    ...
    else if (strcmp((char *)z->key, (char *)tree->key) > 0)// 待删除的节点在"tree的右子树"中
    {
    ...
    else    // tree是对应要删除的节点。
    {
        ...
        if ((tree->left) && (tree->right))
        {
            if (HEIGHT(tree->left) > HEIGHT(tree->right))
            {
				...
				strcpy((char *)tree->key, (char *)max->key);
            ...
            else
            {
				...
				strcpy((char *)tree->key, (char *)min->key);
...
void print_avltree(AVLTree tree, KType key, int direction)
{
    if(tree != NULL)
    {
        if(direction==0)    // tree是根节点
            ...
            printf("%2s is %s's %6s child\n", (char *)tree->key, (char *)key, direction==1?"right" : "left");
